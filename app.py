...
def render_passive_sentinel_inlined():
    ...
    with col_toggle_switch:
        ...
        st.session_state.sentinel_enabled = st.toggle("ON" if st.session_state.sentinel_enabled else "OFF", 
                                                      ...
                                                      key='sentinel_toggle_main_inlined') # Unique key
    ...
    if st.session_state.sentinel_enabled:
        ...
        if st.session_state.stress_probability > 70:
            ...
            with col_btn2:
                if st.button("Dismiss (snooze 10m)", use_container_width=True, key="sentinel_btn_dismiss"):
    ...
    for icon, title, desc, col in sensors:
        with col:
            ...
            """, unsafe_allow_html=True)
            st.checkbox(f'Enable {title}', value=True, key=f'sensor_inlined_{title.replace(" ", "_")}') # Unique Key
    ...
    with col1:
        if st.button("Low", use_container_width=True, type="primary" if st.session_state.sentinel_sensitivity == 'Low' else "secondary", key="sens_low"):
            st.session_state.sentinel_sensitivity = 'Low'
    with col2:
        if st.button("Medium", use_container_width=True, type="primary" if st.session_state.sentinel_sensitivity == 'Medium' else "secondary", key="sens_med"):
            st.session_state.sentinel_sensitivity = 'Medium'
    with col3:
        if st.button("High", use_container_width=True, type="primary" if st.session_state.sentinel_sensitivity == 'High' else "secondary", key="sens_high"):
