...
def render_dashboard():
    ...
    # --- INITIALIZE AI DATA (Moved to top to prevent NameError) ---
    def get_ai_predictions(sri):
        """Returns deterministic AI insights based on the SRI score."""
        # Continuous calculation for recovery time (Inverse to SRI)
        recovery_val = max(2.0, 14.0 - (sri * 0.12)) + random.uniform(-0.5, 0.5)
    ...
    # 1. AI Resilience Prediction Card
    st.markdown(f'<div style="background: linear-gradient(90deg, #1e1b4b 0%, #312e81 100%); border-radius: 12px; padding: 25px; margin-bottom: 20px; border: 1px solid rgba(129, 140, 248, 0.2); box-shadow: 0 4px 20px rgba(0,0,0,0.2);"><div style="display: flex; justify-content: space-between; align-items: flex-start; margin-bottom: 20px;"><div style="display: flex; gap: 15px; align-items: center;"><div style="background: rgba(129, 140, 248, 0.2); width: 45px; height: 45px; border-radius: 50%; display: flex; justify-content: center; align-items: center; color: #a5b4fc; font-size: 1.4rem;">⚡</div><div><div style="color: white; font-weight: 700; font-size: 1.2rem;">AI Resilience Prediction</div><div style="color: #a5b4fc; font-size: 0.9rem;">Based on your patterns and behavioral inputs</div></div></div></div><div style="display: flex; justify-content: space-between; margin-bottom: 8px; font-size: 0.85rem; color: #a5b4fc; font-weight: 600;"><span>Predicted SRI</span><span>Confidence</span></div><div style="display: flex; align-items: center; gap: 20px; margin-bottom: 25px;"><div style="font-size: 2.2rem; font-weight: 800; color: #e0e7ff; line-height: 1;">59</div><div style="flex-grow: 1; height: 10px; background: rgba(255,255,255,0.1); border-radius: 5px;"><div style="width: 83%; height: 100%; background: linear-gradient(90deg, #818cf8 0%, #c084fc 100%); border-radius: 5px; box-shadow: 0 0 10px rgba(129, 140, 248, 0.5);"></div></div><div style="color: white; font-weight: 700; font-size: 1.1rem;">83%</div></div><div style="background: rgba(0,0,0,0.2); border-radius: 8px; padding: 15px; display: flex; gap: 15px; align-items: center; border-left: 3px solid #fbbf24;"><span style="color: #fbbf24; font-size: 1.2rem;">💡</span><span style="color: #e0e7ff; font-size: 0.95rem;"><strong>Recommendation:</strong> Consider a 10-minute breathing exercise before bed to optimize recovery slope.</span></div></div>', unsafe_allow_html=True)

    # 2. Did You Know? Section
    if 'fact_idx' not in st.session_state: st.session_state.fact_idx = 0
    facts = [
        {"t": "Hydration", "d": "Drinking 500ml of water can increase HRV within 10 minutes by improving blood flow and reducing sympathetic activation."},
        {"t": "The Sigh", "d": "A physiological sigh (double inhale, long exhale) is the fastest way to offload CO2 and reset the nervous system."},
        {"t": "Morning Light", "d": "Viewing sunlight within 30 mins of waking anchors your cortisol peak, improving sleep quality 16 hours later."},
        {"t": "Vagal Tone", "d": "Cold water exposure to the face activates the 'Mammalian Dive Reflex', instantly lowering heart rate."}
    ]
    curr_fact = facts[st.session_state.fact_idx]
    ...
    # 3. AI Coach Overlay (Floating)
    st.markdown('<div style="position: fixed; bottom: 30px; right: 30px; width: 320px; background: #0f3d3e; border: 1px solid #14b8a6; border-radius: 12px; padding: 20px; box-shadow: 0 10px 40px rgba(0,0,0,0.6); z-index: 9999; font-family: \'Inter\', sans-serif;"><div style="display: flex; align-items: center; gap: 10px; margin-bottom: 12px;"><div style="background: rgba(20, 184, 166, 0.2); width: 32px; height: 32px; border-radius: 50%; display: flex; justify-content: center; align-items: center; font-size: 1.2rem;">🍃</div><div style="color: #2dd4bf; font-weight: 700; font-size: 0.95rem;">AI Coach</div><div style="margin-left: auto; color: #5eead4; cursor: pointer; font-size: 1.2rem; opacity: 0.7;">&times;</div></div><div style="color: white; font-weight: 700; font-size: 1rem; margin-bottom: 8px;">Maintain Balance</div><div style="color: #ccfbf1; font-size: 0.85rem; line-height: 1.5; margin-bottom: 18px;">Good resilience! A quick coherent breathing session (1-2 min) can push you into optimal range.</div><div style="display: flex; gap: 10px;"><button style="background: #2dd4bf; border: none; padding: 8px 16px; border-radius: 6px; color: #0f172a; font-weight: 700; font-size: 0.8rem; cursor: pointer; flex: 1;">Quick Session</button><button style="background: transparent; border: 1px solid #2dd4bf; padding: 8px 16px; border-radius: 6px; color: #2dd4bf; font-weight: 700; font-size: 0.8rem; cursor: pointer; flex: 1;">Next Tip</button></div><div style="display: flex; justify-content: center; gap: 6px; margin-top: 15px;"><div style="width: 6px; height: 6px; background: #2dd4bf; border-radius: 50%;"></div><div style="width: 6px; height: 6px; background: #115e59; border-radius: 50%;"></div><div style="width: 6px; height: 6px; background: #115e59; border-radius: 50%;"></div></div></div>', unsafe_allow_html=True)
